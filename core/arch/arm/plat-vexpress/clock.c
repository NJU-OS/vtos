...
void clock_set_enable(enum periph_id periph_id, int enable)
{
    ...
    uint32_t *clk = &clkrst->crc_clk_out_enb[PERIPH_REG(periph_id)];
    uint32_t reg;
...
__weak struct clk_pll_simple *clock_get_simple_pll(enum clock_id clkid __unused)
{
	return NULL;
}
		...
		u32 divp, u32 cpcon, u32 lfcon)
{
    ...
    // not used now, we want to enable clock
		...
		enum clock_id parent, unsigned rate, int *extra_div)
{
	...
	if (rate != effective_rate)
		DMSG("Requested clock rate %u not honored (got %u)\n",
			rate, effective_rate);
...
{
    uint32_t delayed = us_delay*1000;
...
int clock_verify(void)
{
	...
	if (!pllp_valid) {
		DMSG("Warning: PLLP %x is not correct\n", reg);
		return -1;
	}
	DMSG("PLLP %x is correct\n", reg);
	return 0;
}
...
int clock_external_output(int clk_id)
{
	...
	if (clk_id >= 1 && clk_id <= 3) {
		setbits_le32(&pmc->pmc_clk_out_cntrl, 1 << (2 + (clk_id - 1) * 8));
	} else {
		DMSG("%s: Unknown output clock id %d\n", __func__, clk_id);
		return -EINVAL;
	}
